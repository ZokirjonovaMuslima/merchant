...
async def answer_password(message: types.Message, state: FSMContext):
    ...
    d.update({"is_client": True, "password": str(message.text)})
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            f"""INSERT INTO users(name, username, balance, password, is_client)VALUES(%s, %s, %s, %s, %s);""",
            (d.get("full_name"), d.get("username"), d.get("balance"), _hash, d.get("is_client")))
        connection.commit()
    make_history()
...
async def answer_fullname(message: types.Message, state: FSMContext):
    ...
    d_m.update({"is_client": False, "merchant_password": str(message.text)})
...
async def product(message: types.Message, state: FSMContext):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            f"""INSERT INTO users(name, username, balance, password, is_client) VALUES (%s, %s, %s, %s, %s);""",
            (d_m.get("name"), d_m.get("merchant_username"), d_m.get("merchant_balance"), hash,
             d_m.get("is_client")))
